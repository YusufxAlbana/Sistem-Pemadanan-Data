...
const DashboardStats = ({ stats, eliminationDetails }) => {
  ...
  const barData = eliminationDetails.map(detail => ({
    ...
    'Jumlah Eliminasi': detail.eliminated
  }));
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="grid-3">
        ...
        <div className="stat-card">
          <div className="stat-icon danger"><FileMinus size={24} /></div>
          <div className="stat-content">
            <h4>Total Tereliminasi</h4>
            <div className="stat-value" style={{ color: 'var(--danger)' }}>{totalEliminated}</div>
          </div>
        </div>
        <div className="stat-card">
              ...
              {barData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="Jumlah Eliminasi" fill="#EF4444" radius={[4, 4, 0, 0]}>
